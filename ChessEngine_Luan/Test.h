...
int ReadFEN(std::string FEN)
{
    ...
    cstl[DUMMY - WHITE] = 0;
    cc = 0;
    while (c = c = p[currentIndex])
    {
        ++currentIndex;
        if (c >= '0' && c <= '9') continue; /* ignore move counts */
        if (c >= 'a' && c <= 'h') /* might be e.p. square */
        {
            if (p[currentIndex] == '3' || p[currentIndex] == '6')
            {
                epSqr = 0x22 + (p[currentIndex] - '1') * 16 + (c - 'a');
                p[currentIndex]++;
                continue;
            }
...
void move_gen(int color, int lastply, int d)
{
    ...
    int savsp, mask, new1, forward, rank, prank, xcolor, ep_flag;
...
{   /* recursive perft, with in-lined make/unmake */
    ...
    if (d <= Split && d > 1)
    {
        ...
        sprintf_s(buf, "%10lld", count - ocnt);
        printf("%d. ", d);
        fprintf(f, "%d. ", d);
        for (i = 1; i < d; i++) {
            printf("%c%c%c%c ",
...
std::string FEN = std::string("r3k2r/8/3Q4/8/8/5q2/8/R3K2R b KQkq - 0 1");
std::string Fritz = std::string("r3r1k1/1pq2pp1/2p2n2/1PNn4/2QN2b1/6P1/3RPP2/2R3KB b - -");
int Dep = 5;
